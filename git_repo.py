...
class MainModel(Observable):
    ...
    def update_status(self, update_index=False):
        ...
        self.notify_observers(self.message_about_to_update)
        self._update_files(update_index=update_index)
        #self._update_refs() # modified by Danny
...
#---------------------------------------------------------------------------
# Model methods create by Danny
#---------------------------------------------------------------------------

    def gitProcess(self, cmd, repoDir):
    
        p = subprocess.Popen(cmd, cwd=repoDir, stdin=subprocess.PIPE, 
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        output, errNote = p.communicate()
        return_code = p.wait()

        return output + errNote # p.returncode


    def is_in_git_repo(self, p):
    
        self.set_worktree(p)

        if os.path.basename(self.git.git_dir()) == '.git':
            return True
        else:
            return False


    def is_git_dir_clean(self, p):
