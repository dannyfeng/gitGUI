...
def fldr_status(p):
    '''
    Return the status of a git repository file
    '''
    pass
...
# this function shoud be modified(Danny)
def specified_diff_upstream(head, p):
    tracked = tracked_branch()
    if not tracked:
        return []
    merge_base = merge_base_to(head, tracked)
...
#def specified_dir_state(head='HEAD', p=git.git_dir, update_index=False):
#    pass
#---------------------------------------------------------------------------
